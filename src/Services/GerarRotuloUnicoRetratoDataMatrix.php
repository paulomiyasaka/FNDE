...
class GerarRotuloUnicoRetratoDataMatrix {
    private $mpdf;
    ...
    /**
     * Cria o DataMatrix de altíssima densidade para suportar strings gigantes.
     * Ajustado o pixel ratio para (7, 7) garantindo que os micro-quadrados fiquem nítidos no papel.
     */
    private function gerarDataMatrixBase64($stringDados) {
        $barcodeObj = new TCPDF2DBarcode($stringDados, 'DATAMATRIX');
        $pngData = $barcodeObj->getBarcodePngData(7, 7); 
     ...
     */
    public function renderizar($dadosGears, array $paletes) {
        ...
        foreach ($paginasDeCarga as $index => $paletesDaPagina) {
            ...
            $stringCompletaMaster = $dadosGears['qr_master'];
            ...
            // 3. GERAÇÃO DO CÓDIGO FONTE MÃE COM A STRING CONCATENADA
            $linkDataMatrixUnificado = $this->gerarDataMatrixBase64($stringCompletaMaster);
            ...
            <div class='header-container'>
                <table class='tabela-cabecalho'>
                    ...
                    <tr>
                        <td colspan='3' class='saida-bold'>{$dadosGears['destino']}</td>
                        <td class='saida-bold' style='text-align: center;'>{$dadosGears['se']}</td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td class='saida-bold' style='text-align: center;'>{$dadosGears['sigla']}</td>
                        <td class='saida-bold' style='text-align: center;'>{$dadosGears['qtd_total']}</td>
                        <td class='saida-bold' style='text-align: center;'>{$dadosGears['peso_total']} kg</td>
            ...
            <div class='centro-container'>
                ...
                <div class='wrapper-qr'>
                    <img src='{$linkDataMatrixUnificado}' class='qr-unificado-img'>
                </div>
            </div>
        ...
        $this->mpdf->Output('Rotulo_Unico_Expedicao.pdf', 'I');
    }
